...
public class SpecificationTemplate {
    ...
    public static Specification<PaymentModel> paymentUserId(final UUID userId) {
        return (root, query, cb) -> {
          query.distinct(true);
          final Root<PaymentModel> payment = root;
          final Root<UserModel> user = query.from(UserModel.class);
          final Expression<Collection<PaymentModel>> usersPayment = user.get("payments");

          return cb.and(cb.equal(user.get("userId"), userId), cb.isMember(payment, usersPayment));
        };
    }
